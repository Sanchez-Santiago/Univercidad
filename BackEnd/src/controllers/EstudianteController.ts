    ...
    >
{
  ...
  async create(params: { data: EstudianteCreateInput }): Promise<Estudiante> {
    try {
      ...
      const password = await bcrypt.hash(validData.password);
      ...
      if (await this.model.getByEmail({ email: full.email })) {
  ...
  async update(params: {
    ...
    try {
      ...
      if (params.data.password) {
        validated.password = await bcrypt.hash(params.data.password);
      }
