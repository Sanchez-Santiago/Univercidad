...
import { ProfesorController } from "../controllers/ProfesorController.ts";
import { ModelDB } from "../interfaces/Model.ts";
import { Profesor, ProfesorCreateInput } from "../schemas/profesor.ts";

type ExtractParams<T, R extends string> = (
  ctx: RouterContext<R>
) => Promise<T> | T;

// Instancia del controladorconst
function RouterProfesor(profesorModel: ModelDB<Profesor>) {
  const estudiante = new ProfesorController(profesorModel);
  ...
  estudianteRouter
    ...
    .post("/estudiantes", async (ctx) => {
      try {
        ...
        const data = (await body) as ProfesorCreateInput;
